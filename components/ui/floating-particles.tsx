...
export function FloatingParticles({
  ...
  useEffect(() => {
    ...
    const updateParticles = () => {
      const positionsArray = pointsGeometry.attributes.position
        .array as Float32Array;
      const positionsArray2 = pointsGeometry2.attributes.position
        .array as Float32Array;
      ...
      for (let i = 0; i < movers.length; i++) {
        ...
        if (mover.is_active) {
          ...
          if (mover.position.y > 1000) {
            const range =
              (Math.log(getRandomInt(2, 256)) / Math.log(256)) * 250 + 50;
            const rad = getRadian(getRandomInt(0, 360));
            const x = Math.cos(rad) * range;
            const z = Math.sin(rad) * range;
